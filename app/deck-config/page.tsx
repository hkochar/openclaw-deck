...
export default function DeckConfigPage() {
  ...
  type EditTab = "agents" | "channels" | "infra" | "sentinel" | "dashboard" | "budgets" | "providers" | "replay";
  type SourceTab = "config/deck-agents.json" | "config/deck-config.json" | "sentinel/deck-sentinel.json";

  const EDIT_TABS: EditTab[] = ["budgets", "agents", "channels", "providers", "infra", "sentinel", "dashboard", "replay"];
  ...
  for (const section of ["systemChannels", "pluginChannels"] as const) {
    for (const [name, id] of Object.entries(config[section])) {
      ...
      } else if (!isSnowflake(id)) {
        clientErrors.push(`${section} #${name}: invalid channel ID`);
      }
    }
  }
  // Log channels: must have valid IDs
  for (const [name, id] of Object.entries(config.logChannels)) {
    if (!id.trim() || !isSnowflake(id)) clientErrors.push(`Log #${name}: invalid channel ID`);
  }
        ...
        <>
          ...
          {channelWarnings.length > 0 && (
            <div className="mcc-warning-bar">
              Missing channel IDs: {channelWarnings.join(", ")} &mdash; Discord notifications will not work until configured.
            </div>
          )}
                  ...
                  <tbody>
                    {Object.entries(config[key]).map(([name, id]) => (
                      <tr key={name} className="mcc-row">
                        ...
                        <td data-label="ID">
                          <input
                            className={`mcc-input mcc-input--mono${id.trim() && !isSnowflake(id) ? " mcc-input--error" : ""}${!id.trim() ? " mcc-input--warn" : ""}`}
                            value={id}
              ...
              <div className="mcc-table-wrap">
                <table className="mcc-table mcc-table--kv">
                  ...
                  <tbody>
                    ...
                    <tr className="mcc-row">
                      <td data-label="Setting">Alert Channel</td>
                      <td data-label="Value">
                        {Object.keys(config.systemChannels ?? {}).length > 0 ? (
                          <div className="mcc-radio-group mcc-radio-group--inline">
                            {Object.keys(config.systemChannels ?? {}).map((ch) => (
                              <label key={ch} className={`mcc-radio-option${(budgets?.alertChannel ?? "systemStatus") === ch ? " mcc-radio-option--active" : ""}`}>
                                <input type="radio" name="budgetAlertChannel" value={ch} checked={(budgets?.alertChannel ?? "systemStatus") === ch}
                                  onChange={() => updateBudgets({ alertChannel: ch })} />
                                <span className="mcc-radio-label">#{ch.replace(/([A-Z])/g, "-$1").toLowerCase()}</span>
                              </label>
                            ))}
                          </div>
                        ) : (
                          <span style={{ fontSize: 12, color: "var(--text-muted)" }}>No system channels configured — add them in the Channels tab</span>
                        )}
                      </td>
                    </tr>
                    <tr className="mcc-row">
