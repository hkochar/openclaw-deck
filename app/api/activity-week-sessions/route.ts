...
function queryDirect(dateStr: string): ActivityChunk[] | null {
  ...
  try {
    ...
    // Look up session metadata for display info
    ...
    // Group events by agent (use event.agent directly)
    ...
    for (const e of events) {
      if (!e.agent) continue;
      const arr = eventsByAgent.get(e.agent) ?? [];
      arr.push(e);
      eventsByAgent.set(e.agent, arr);
    }
...
export async function GET(req: NextRequest) {
  ...
  if (!datePattern.test(start) || !datePattern.test(end)) {
    return NextResponse.json({ error: "Missing start/end params (YYYY-MM-DD)" }, { status: 400 });
  }

  // Try gateway first
  try {
    const res = await fetch(`${GATEWAY_URL}/activity/week-sessions?start=${start}&end=${end}`, {
  ...
  // Direct SQLite fallback — iterate each day in range
  const result: Record<string, ActivityChunk[]> = {};
  const startDate = new Date(start + "T00:00:00");
  const endDate = new Date(end + "T00:00:00");

  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    const ds = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
    const chunks = queryDirect(ds);
    result[ds] = chunks ?? [];
  }
