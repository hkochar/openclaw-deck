...
const plugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    const systemStatusChannelId = deckConfig?.systemChannels?.systemStatus ?? "";

    // Stuck loop: alert to system-status
    onLoopDetected((agent, tool, count, signature) => {
      discord.sendLoopAlert(agent, tool, count, systemStatusChannelId, signature);
      logEvent({
        agent,
        type: "loop_detected",
    ...
    // Agent silence: alert to system-status
    onAgentSilence((agent, silenceMinutes) => {
      discord.sendAgentSilenceAlert(agent, silenceMinutes, systemStatusChannelId);
      logEvent({
        agent,
        type: "agent_silence",
    ...
    // Replay alerts: route to Discord (uses global budgets.alertChannel)
    onReplayAlert((event) => {
      const budgetCfg = loadBudgetConfig();
      const channelKey = budgetCfg.alertChannel || "systemStatus";
      const channelId = deckConfig?.systemChannels?.[channelKey] || systemStatusChannelId;
      discord.sendReplayAlert(event, channelId);
    });
          ...
          try {
            ...
            if (job) {
              ...
              if (!event.success || job.state?.lastStatus === "error") {
                ...
                if (errors >= 2) {
                  discord.sendCronFailureAlert(agentKey, job.name, errorMsg, errors);
                }
              } else if (prevErrors >= 2 && errors === 0) {
                discord.sendCronRecoveryAlert(agentKey, job.name);
                logEvent({
                  agent: agentKey,
                  session: ctx.sessionKey,
                  type: "cron_recovery",
    ...
    api.on("before_model_resolve", async (event, ctx) => {
      ...
      if (budgetResult.action === "block") {
        ...
        const config = loadBudgetConfig();
        const alertChannelId = deckConfig?.systemChannels?.[config.alertChannel] ?? "";
        const botToken = deckBotToken || discord.botToken;
        sendBudgetAlert(agentKey, "blocked", alertChannelId, botToken, { trigger: budgetResult.trigger, period: budgetResult.period, ratio: budgetResult.ratio }).catch(() => {});
      ...
      if (budgetResult.action === "throttle") {
        ...
        if (throttled) {
          ...
          const alertChannelId = deckConfig?.systemChannels?.[config.alertChannel] ?? "";
          const botToken = deckBotToken || discord.botToken;
          sendBudgetAlert(agentKey, "exceeded", alertChannelId, botToken, { trigger: budgetResult.trigger, period: budgetResult.period, ratio: budgetResult.ratio }).catch(() => {});
          if (ctx.sessionKey) {
            enforcementState.set(ctx.sessionKey, {
      ...
      if (budgetResult.action === "alert") {
        ...
        const alertChannelId = deckConfig?.systemChannels?.[config.alertChannel] ?? "";
        const botToken = deckBotToken || discord.botToken;
        sendBudgetAlert(agentKey, "threshold", alertChannelId, botToken, { trigger: budgetResult.trigger, period: budgetResult.period, ratio: budgetResult.ratio }).catch(() => {});
      }
      ...
      try {
        ...
        for (const w of providerWindows) {
          for (const t of thresholds) {
            if (w.pct >= t) {
              sendProviderLimitAlert(w.provider, w, alertChannelId, botToken).catch(() => {});
              logEvent({
                agent: w.provider,
    ...
    api.on("llm_output", async (event, ctx) => {
      ...
      if (isCron) {
        if (modelsMatch(actualModel, configuredPrimary)) {
          if (hasUnresolvedDrift(agentKey)) {
            ...
            discord.sendDriftResolved(driftLabel, configuredPrimary);
          }
        } else {
          reportDriftDb(agentKey, configuredPrimary, actualModel, "cron");
          discord.sendDriftAlert(driftLabel, configuredPrimary, actualModel, "cron");
      ...
      discord.sendDriftAlert(driftLabel, configuredPrimary, actualModel, tag);
