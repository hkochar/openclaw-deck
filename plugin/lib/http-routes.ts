...
import {
  ...
  fireReplayAlert, resolveAlertChannel,
  type ReplayAlertEvent, type AlertTokens,
} from "../budget";
...
export interface HttpRouteContext {
  ...
  deckBotToken: string;
  discord: { botToken: string; tokens: AlertTokens; sendReplayAlert(event: unknown, channelId: string): void; sendRawAlert(channelRef: string, msg: { title: string; lines: string[]; icon?: string; buttons?: Array<{ label: string; url: string }> }): void };
    ...
    handler: (req: IncomingMessage, res: ServerResponse) => {
      ...
      req.on("end", () => {
        try {
          ...
          // Send alert for pause/resume
          const alertChannelRef = resolveAlertChannel("budget", ctx.deckConfig);
          if (alertChannelRef) {
            sendBudgetAlert(agent, paused ? "paused" : "resumed", alertChannelRef, ctx.discord.tokens, {
        ...
        try {
          ...
          if (req.method === "POST") {
            ...
            if (alertChannelRef) {
              ...
              ctx.discord.sendRawAlert(alertChannelRef, {
                title: `Budget Override: ${agent}`,
                lines: [
                  `Agent:     ${agent}`,
      ...
      req.on("end", async () => {
        try {
          ...
          if (!alertChannelRef) {
            ...
            res.setHeader("Content-Type", "application/json");
            setCorsOrigin(req, res);
            res.end(JSON.stringify({ ok: true, sent: false, reason: "no alert channel configured" }));
            return;
          }
          ...
          await sendBudgetAlert(agent, alertLevel, alertChannelRef, ctx.discord.tokens, {
